...
// Полная реализация слайса каналов (перенесена из src/slices/channelsSlice.js)
...
export const addChannel = createAsyncThunk(
  ...
  async (name, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.post('/channels', { name: filteredName })
...
export const renameChannel = createAsyncThunk(
  ...
  async ({ id, name }, { rejectWithValue }) => {
    try {
      const filteredName = cleanProfanity(name)
      const response = await axiosInstance.patch(`/channels/${id}`, { name: filteredName })
      return response.data
    }
    catch (error) {
      handleNetworkError(error)
...
const channelsSlice = createSlice({
  ...
  reducers: {
    ...
    socketRemoveChannel: (state, action) => {
      ...
      const channelName = state.channels.find(ch => ch.id === channelId)?.name
