...
const MessagesList = () => {
  ...
  const channelMessages = messages.filter(
    (msg) => Number(msg.channelId) === Number(currentChannelId) // Приводим к Number для надежности
  );

  return (
    <div className="col-8 d-flex flex-column h-100">
      <div className="p-3 border-bottom">
        <h5 className="mb-0">Сообщения</h5>
      </div>
      <ListGroup variant="flush" className="flex-grow-1 overflow-auto p-3">
        {channelMessages.map((message) => (
          <ListGroup.Item key={message.id} className="border-0 px-0">
            <strong>{message.username}:</strong> {message.body || message.text} {/* Поддерживаем оба варианта */}
          </ListGroup.Item>
        ))}
